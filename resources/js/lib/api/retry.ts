...
export const toRetryPolicy = (policy?: RetryPolicy): Required<RetryPolicy> => ({
    ...defaultRetryPolicy,
    ...policy,
    retryOnStatuses: policy?.retryOnStatuses ?? defaultRetryPolicy.retryOnStatuses,
});
...
export const getBackoffDelay = (attemptIndex: number, baseDelayMs: number, maxDelayMs: number) => {
