...
const onOutsideClick = (event: MouseEvent): void => {
    ...
    if (
        ...
        !algorithmMenuRef.value.contains(event.target)
    ) {
...
const selectAlgorithm = (
    value: (typeof algorithmOptions)[number]['value']
): void => {
    ...
    <div class="flex min-h-0 flex-1 flex-col gap-4">
        ...
        >
            ...
            <div class="mt-3 flex items-center gap-3">
                <button
                    :disabled="!canRun"
                    class="inline-flex h-10 items-center gap-2 rounded-md bg-primary px-5 text-sm font-semibold text-primary-foreground disabled:opacity-60"
                    @click="run"
                >
                    <LoaderCircle
                        v-if="loading"
