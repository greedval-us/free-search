...
const isTransformCipher = computed(() =>
    transformCiphers.includes(cipher.value)
);
...
onMounted(() => {
    ...
    const morseSeparatorValue = readRepeatQueryParam(params, [
        'morse_separator',
    ]);
    ...
    if (
        directionValue === 'encrypt' ||
        directionValue === 'decrypt' ||
        directionValue === 'transform'
    ) {
    ...
    if (railsValue !== null) {
        rails.value = railsValue;
    }

    if (xorKeyValue !== '') {
        xorKey.value = xorKeyValue;
    }

    if (affineAValue !== null) {
        affineA.value = affineAValue;
    }

    if (affineBValue !== null) {
        affineB.value = affineBValue;
    }

    if (playfairKeyValue !== '') {
        playfairKey.value = playfairKeyValue;
    }

    if (columnKeyValue !== '') {
        columnKey.value = columnKeyValue;
    }

    if (morseSeparatorValue !== '') {
        morseSeparator.value = morseSeparatorValue;
    }
    ...
    <div class="flex min-h-0 flex-1 flex-col gap-4">
        ...
        >
            ...
            <textarea
                ...
                :placeholder="t('shifr.input.placeholder')"
            />
            ...
            <div class="mt-3 grid gap-3 md:grid-cols-3">
                <label>
                    ...
                    >
                        <option value="caesar">
                            {{ t('shifr.classic.caesar') }}
                        </option>
                        <option value="atbash">
                            {{ t('shifr.classic.atbash') }}
                        </option>
                        <option value="rot13">
                            {{ t('shifr.classic.rot13') }}
                        </option>
                        <option value="rot47">
                            {{ t('shifr.classic.rot47') }}
                        </option>
                        <option value="rot5">
                            {{ t('shifr.classic.rot5') }}
                        </option>
                        <option value="vigenere">
                            {{ t('shifr.classic.vigenere') }}
                        </option>
                        <option value="rail_fence">
                            {{ t('shifr.classic.railFence') }}
                        </option>
                        <option value="xor">
                            {{ t('shifr.classic.xor') }}
                        </option>
                        <option value="affine">
                            {{ t('shifr.classic.affine') }}
                        </option>
                        <option value="playfair">
                            {{ t('shifr.classic.playfair') }}
                        </option>
                        <option value="columnar">
                            {{ t('shifr.classic.columnar') }}
                        </option>
                        <option value="morse">
                            {{ t('shifr.classic.morse') }}
                        </option>
                    </select>
                </label>
                <label>
                    <span
                        class="mb-1 block text-xs font-medium text-muted-foreground"
                        >{{ t('shifr.classic.direction') }}</span
                    >
                    <select
                        v-model="direction"
                        class="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                    >
                        <option v-if="!isTransformCipher" value="encrypt">
                            {{ t('shifr.classic.encrypt') }}
                        </option>
                        <option v-if="!isTransformCipher" value="decrypt">
                            {{ t('shifr.classic.decrypt') }}
                        </option>
                        <option v-if="isTransformCipher" value="transform">
                            {{ t('shifr.classic.transform') }}
                        </option>
                    </select>
                </label>
            ...
            <div class="mt-3 grid gap-3 md:grid-cols-2">
                ...
                <label v-if="showAffine">
                    <span
                        class="mb-1 block text-xs font-medium text-muted-foreground"
                        >{{ t('shifr.classic.affineA') }}</span
                    >
                    <input
                        v-model.number="affineA"
                        type="number"
                        min="1"
                        max="1000"
                        class="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                ...
                <label v-if="showAffine">
                    <span
                        class="mb-1 block text-xs font-medium text-muted-foreground"
                        >{{ t('shifr.classic.affineB') }}</span
                    >
                    <input
                        v-model.number="affineB"
                        type="number"
                        min="-1000"
                        max="1000"
                        class="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                ...
                <label v-if="showPlayfairKey">
                    <span
                        class="mb-1 block text-xs font-medium text-muted-foreground"
                        >{{ t('shifr.classic.playfairKey') }}</span
                    >
                    <input
                        v-model="playfairKey"
                        type="text"
                ...
                <label v-if="showColumnKey">
                    <span
                        class="mb-1 block text-xs font-medium text-muted-foreground"
                        >{{ t('shifr.classic.columnKey') }}</span
                    >
                    <input
                        v-model="columnKey"
                        type="text"
                ...
                <label v-if="showMorseSeparator">
                    <span
                        class="mb-1 block text-xs font-medium text-muted-foreground"
                        >{{ t('shifr.classic.morseSeparator') }}</span
                    >
                    <input
                        v-model="morseSeparator"
                        type="text"
                        maxlength="5"
                        class="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                    />
