...
<template>
    <section class="intel-panel flex min-h-0 flex-1 flex-col">
        <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <div
                v-if="hasResult"
                class="inline-flex rounded-md border border-sidebar-border/70 bg-background/40 p-0.5 text-xs"
            >
                <button
                    type="button"
        ...
        <div
            class="intel-scroll min-h-0 flex-1 overflow-auto rounded-md border border-sidebar-border/60 bg-background/40 p-3"
        >
            <div v-if="result && mode === 'readable'" class="space-y-2 text-sm">
                ...
                >
                    ...
                    <p v-if="isPrimitive(value)" class="mt-1 break-words">
                        {{ value }}
                    </p>
                    <ul
                    ...
                    <pre
                        v-else
                        class="mt-1 text-xs leading-5 break-words whitespace-pre-wrap"
                        >{{ JSON.stringify(value, null, 2) }}</pre
                    >
                </div>
            </div>
            <pre v-else-if="result" class="text-xs leading-6">{{
