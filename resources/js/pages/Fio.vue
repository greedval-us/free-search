...
<template>
    ...
    >
        <section class="intel-panel sticky top-0 z-10 shrink-0">
        ...
        <section class="intel-panel flex min-h-0 flex-1 flex-col">
            ...
            >
                <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
                    ...
                    >
                        <p class="text-xs text-muted-foreground">
                            {{ t('fio.lookup.domains') }}
                        </p>
                        <p class="mt-1 text-xl font-semibold">
                            {{ result.summary.domains }}
                        </p>
                    </div>
                    <div
                        class="rounded-lg border border-border/70 bg-background/60 p-3"
                    >
                        <p class="text-xs text-muted-foreground">
                            {{ t('fio.lookup.topRegion') }}
                        </p>
                        <p class="mt-1 text-sm font-semibold">
                            {{ regionLabel(result.summary.topRegion) }}
                        </p>
                    </div>
                    <div
                        class="rounded-lg border border-border/70 bg-background/60 p-3"
                    >
                        <p class="text-xs text-muted-foreground">
                            {{ t('fio.lookup.medianAge') }}
                        </p>
                        <p class="mt-1 text-sm font-semibold">
                            {{ result.summary.medianAge ?? '-' }}
                        </p>
                    </div>
                    <div
                        class="rounded-lg border border-border/70 bg-background/60 p-3"
                    >
                        <p class="text-xs text-muted-foreground">
                            {{ t('fio.lookup.averageConfidence') }}
                        </p>
                        <p class="mt-1 text-sm font-semibold">
                ...
                >
                    ...
                    <p class="mt-1">
                        ...
                        <span class="text-muted-foreground">{{
                            result.target.qualifier || '-'
                        }}</span>
                    </p>
                    <p class="mt-1">
                        {{ t('fio.lookup.qualifierTerms') }}:
                        <span class="text-muted-foreground">{{
                            (result.target.qualifierTerms ?? []).join(', ') ||
                            '-'
                        }}</span>
                    </p>
                    <p class="mt-1">
                        ...
                        <span class="text-muted-foreground">{{
                            result.summary.qualifierMatches
                        }}</span>
                    </p>
                </div>
                ...
                >
                    ...
                    <div
                        v-if="(result.source.stats ?? []).length === 0"
                    ...
                    <div v-else class="space-y-2">
                        <div
                            v-for="stat in result.source.stats"
                            :key="stat.source"
                        >
                            ...
                            <div class="h-2 rounded bg-slate-700/70">
                                <div
                                    class="h-2 rounded bg-violet-400"
                                    :style="{
                ...
                >
                    ...
                    <div
                        v-if="
                            (result.source.attemptedSources ?? []).length === 0
                        "
                        class="text-muted-foreground"
                    >
                        -
                ...
                >
                    ...
                    <div
                        v-if="(result.source.sourceErrors ?? []).length === 0"
                    ...
                    <div v-else class="space-y-1">
                        <p
                            v-for="(sourceError, idx) in result.source
                                .sourceErrors"
                ...
                <div class="grid gap-3 xl:grid-cols-2">
                    ...
                    >
                        <p class="mb-2 font-semibold">
                            {{ t('fio.lookup.regionClusters') }}
                        </p>
                        <div
                            v-if="result.clusters.regions.length === 0"
                            class="text-muted-foreground"
                        >
                        ...
                        <div v-else class="space-y-2">
                            <div
                                v-for="cluster in result.clusters.regions"
                                :key="`region-${cluster.key}`"
                            >
                                <div
                    ...
                    >
                        <p class="mb-2 font-semibold">
                            {{ t('fio.lookup.ageClusters') }}
                        </p>
                        <div
                            v-if="result.clusters.ages.length === 0"
                            class="text-muted-foreground"
                        >
                            {{ t('fio.lookup.noClusters') }}
                        </div>
                        <div v-else class="space-y-2">
                            <div
                                v-for="cluster in result.clusters.ages"
                                :key="`age-${cluster.key}`"
                            >
                                <div
                                    class="mb-1 flex items-center justify-between"
                            ...
                            >
                                <div class="min-w-0 flex-1">
                                    ...
                                    <a
                                        ...
                                        class="[overflow-wrap:anywhere] break-words text-cyan-300 hover:underline"
                                    >
                                        {{ item.url }}
                                    </a>
                                </div>
                                <span
                                    class="rounded-full border border-cyan-500/30 bg-cyan-500/10 px-2 py-0.5 text-[11px]"
                                >
                                    {{ t('fio.lookup.confidence') }}:
                                    {{ item.confidence }}%
                                </span>
                            </div>
                            <p
