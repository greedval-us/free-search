...
const historyStageLabel = (value: string | null) => {
    if (!value) {
        return t('mastodon.parser.history.unknown');
    }

    const key = `mastodon.parser.progress.stage.${value}`;
...
<template>
    ...
    >
        ...
        >
            <div class="space-y-1">
                <h3 class="text-base font-semibold">
                    {{ t('mastodon.parser.history.title') }}
                </h3>
                <p class="text-sm text-muted-foreground">
                    {{
                        t('mastodon.parser.history.description', {
                            days: retentionDays,
                        })
                    }}
                </p>
            ...
            >
                {{
                    t('mastodon.parser.history.retention', {
                        days: retentionDays,
                    })
                }}
            </div>
        ...
        <div v-if="loading" class="py-6 text-sm text-muted-foreground">
            {{ t('mastodon.parser.history.loading') }}
        </div>
        ...
        >
            {{ t('mastodon.parser.history.empty') }}
        </div>
                    ...
                    <div class="space-y-3">
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="text-sm font-semibold">
                                {{
                                    item.account ||
                                    t('mastodon.parser.history.unknown')
                                }}
                            </span>
                            <span
                        ...
                        >
                            <div>
                                <span class="block">
                                    {{ t('mastodon.parser.history.created') }}
                                </span>
                            ...
                            <div>
                                <span class="block">
                                    {{ t('mastodon.parser.history.expires') }}
                                </span>
                                <span class="text-foreground">
                        ...
                        >
                            ...
                            >
                                {{
                                    t('mastodon.parser.history.statuses', {
                                        count: item.processedStatuses,
                                    })
                                }}
                            </span>
                            <span
                            ...
                            >
                                {{
                                    t('mastodon.parser.history.comments', {
