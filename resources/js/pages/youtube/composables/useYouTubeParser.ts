...
type ParserStage = 'idle' | 'comments' | 'replies' | 'finishing' | 'completed' | 'failed' | 'stopped'
...
export const useYouTubeParser = (t: TranslateFn) => {
  ...
  const canStart = computed(() => form.videoId.trim().length > 0 && !loading.value)
  ...
  const stopSilently = () => {
    ...
    if (activeRunId) {
      apiRequest<ParserStatusResponse>(`/youtube/parser/stop/${activeRunId}`, {
        method: 'POST',
        headers: {
  ...
  const start = async () => {
    ...
    try {
      const response = await apiRequest<ParserStatusResponse>('/youtube/parser/start', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || !response.data.runId) {
        throw new Error(response.message ?? t('youtube.parser.errors.failed'))
      }
      ...
      const pollStatus = async () => {
        ...
        try {
          const statusResponse = await apiRequest<ParserStatusResponse>(`/youtube/parser/status/${runId.value}`, {
            method: 'GET',
          })

          if (!statusResponse.ok) {
            throw new Error(statusResponse.message ?? t('youtube.parser.errors.failed'))
          }
          ...
          if (statusPayload.status === 'completed' || statusPayload.status === 'failed' || statusPayload.status === 'stopped') {
          ...
          loading.value = false
          error.value = pollError instanceof Error ? pollError.message : t('youtube.parser.errors.failed')
          stopSilently()
          return
        } finally {
          pollRequestInFlight.value = false
      ...
      stage.value = 'failed'
      error.value = exception instanceof Error ? exception.message : t('youtube.parser.errors.failed')
    } finally {
