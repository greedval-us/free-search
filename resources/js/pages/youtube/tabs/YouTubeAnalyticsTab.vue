    ...
    <div class="flex items-center justify-between gap-3">
      <div class="space-y-1">
        <div class="flex items-center gap-2 text-sm font-semibold">
          ...
          <HelpTooltip :label="t('youtube.analytics.help.label')" :text="t('youtube.analytics.hint')" />
        </div>
    ...
    <div class="telegram-scroll min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
      ...
      <template v-else>
        ...
        <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-6">
          <MetricCard :title="t('youtube.analytics.metrics.videos')" :value="fmt(result.totals.videos)" :help-text="t('youtube.analytics.metricHints.videos')" />
          <MetricCard :title="t('youtube.analytics.metrics.views')" :value="fmt(result.totals.views)" :help-text="t('youtube.analytics.metricHints.views')" />
          <MetricCard :title="t('youtube.analytics.metrics.likes')" :value="fmt(result.totals.likes)" :help-text="t('youtube.analytics.metricHints.likes')" />
          <MetricCard :title="t('youtube.analytics.metrics.comments')" :value="fmt(result.totals.comments)" :help-text="t('youtube.analytics.metricHints.comments')" />
          <MetricCard :title="t('youtube.analytics.metrics.avgViews')" :value="fmt(result.totals.avgViews)" :help-text="t('youtube.analytics.metricHints.avgViews')" />
          <MetricCard :title="t('youtube.analytics.metrics.engagement')" :value="pct(result.totals.engagementRate)" :help-text="t('youtube.analytics.metricHints.engagement')" />
        </div>

        <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard :title="t('youtube.analytics.metrics.subscribers')" :value="result.channel ? fmt(result.channel.subscriberCount) : '-'" :help-text="t('youtube.analytics.metricHints.subscribers')" />
          <MetricCard :title="t('youtube.analytics.metrics.channelViews')" :value="result.channel ? fmt(result.channel.viewCount) : '-'" :help-text="t('youtube.analytics.metricHints.channelViews')" />
          <MetricCard :title="t('youtube.analytics.metrics.medianViews')" :value="fmt(result.totals.medianViews)" :help-text="t('youtube.analytics.metricHints.medianViews')" />
          <MetricCard :title="t('youtube.analytics.metrics.commentRate')" :value="pct(result.totals.commentRate)" :help-text="t('youtube.analytics.metricHints.commentRate')" />
        </div>

        <YouTubeAnalyticsInsights :insights="result.insights" :insight-label="insightLabel" />

        <section class="grid gap-4 xl:grid-cols-2">
          <YouTubeAnalyticsTimeline :timeline="result.distribution.timeline" :fmt="fmt" />
          <YouTubeAnalyticsDistribution :distribution="result.distribution" />
        </section>

        <YouTubeAnalyticsLeaders :groups="leaderGroups" :format-date="formatDate" :video-metric="videoMetric" />
