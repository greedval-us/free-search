...
<template>
  <section class="grid gap-4 xl:grid-cols-2">
    <SectionCard v-for="group in props.groups" :key="group.key" :title="group.title">
      <template #actions>
        <HelpTooltip :label="t('youtube.analytics.help.label')" :text="t('youtube.analytics.help.leaders')" width-class="w-72" align="right" />
      </template>
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              ...
              <p class="mt-1 text-xs text-muted-foreground">
                {{ props.videoMetric(video, group.key) }} · {{ video.durationLabel || video.duration }} · {{ props.formatDate(video.publishedAt) }}
              </p>
            </div>
