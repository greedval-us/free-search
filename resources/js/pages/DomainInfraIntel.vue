...
defineOptions({
    layout: {
        breadcrumbs: [
            { title: 'Domain & Infra Intel', href: '/domain-infra-intel' },
        ],
    },
});
...
const { loading, error, result, canSearch, lookup } =
    useIntelLookup<DomainInfraResult>(domain, {
        ...
        <IntelResultPanel>
            ...
            >
                <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                    <MetricCard
                        :title="t('domainInfraIntel.summary.domain')"
                        :value="result.domain"
                    />
                    <MetricCard
                        :title="t('domainInfraIntel.summary.ips')"
                        :value="result.ips.join(', ') || '-'"
                    />
                    <MetricCard
                        :title="t('domainInfraIntel.summary.asn')"
                        :value="result.asn?.asn || '-'"
                    />
                    <MetricCard
                        :title="t('domainInfraIntel.summary.org')"
                        :value="result.asn?.org || '-'"
                    />
                </div>

                <SectionCard :title="t('domainInfraIntel.rdap.title')">
                    <p class="mb-2 text-xs text-muted-foreground">
                        {{ t('domainInfraIntel.help.rdap') }}
                    </p>
                    <p class="text-sm">
                        {{ t('domainInfraIntel.rdap.statuses') }}:
                        <span class="text-muted-foreground">{{
                            result.rdap?.statuses?.join(', ') || '-'
                        }}</span>
                    </p>
                    <p class="text-sm">
                        {{ t('domainInfraIntel.rdap.nameservers') }}:
                        <span class="text-muted-foreground">{{
                    ...
                    <p class="text-sm">
                        {{ t('domainInfraIntel.rdap.neighbors') }}:
                        <span class="text-muted-foreground">{{
                            result.neighbors.join(', ') || '-'
                        }}</span>
                    </p>
                </SectionCard>
                ...
                <SectionCard :title="t('domainInfraIntel.certs.title')">
                    <p class="mb-2 text-xs text-muted-foreground">
                        {{ t('domainInfraIntel.help.certs') }}
                    </p>
                    <div v-if="result.crtsh.length === 0" class="intel-empty">
                        {{ t('domainInfraIntel.certs.none') }}
                    </div>
                    <div v-else class="space-y-2">
                        <article
                            v-for="(cert, index) in result.crtsh.slice(0, 20)"
                            :key="`${cert.nameValue}-${index}`"
                            class="intel-surface text-xs"
                        >
                            <p class="font-medium break-all">
                                {{ cert.nameValue }}
                            </p>
                            <p class="text-muted-foreground">
                                {{ t('domainInfraIntel.certs.issuer') }}:
                                {{ cert.issuer || '-' }}
                            </p>
                            <p class="text-muted-foreground">
                                {{ cert.notBefore || '-' }} ->
