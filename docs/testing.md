...
CI отдельно запускает Vitest и проверяет Pint для изменённых PHP-файлов относительно базового коммита PR/push. Это постепенный контроль стиля: старые нарушения вне изменённых файлов не блокируют новые исправления. Полная проверка `composer run lint:check` по-прежнему охватывает весь проект.
