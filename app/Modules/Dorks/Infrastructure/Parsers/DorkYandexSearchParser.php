...
final class DorkYandexSearchParser
{
    /**
    ...
    public function parse(string $html, string $goal, string $dork, int $limit): array
    {
        ...
        $rows = $xpath->query('//li[contains(@class,"serp-item")] | //div[contains(@class,"Organic")]');
        ...
        foreach ($rows as $row) {
            ...
            $snippetNode = $xpath->query('.//div[contains(@class,"OrganicTextContentSpan")] | .//div[contains(@class,"text-container")] | .//p', $row)?->item(0);
    ...
    private function resolveUrl(string $href): string
    {
        ...
        if ((str_contains($host, 'yandex.') || str_contains($host, 'ya.ru')) && str_contains($path, '/clck')) {
            parse_str($query, $params);

            foreach (['url', 'purl', 'target'] as $key) {
